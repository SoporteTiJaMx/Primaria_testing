<?php
	$target_dir = $_SERVER['DOCUMENT_ROOT'] . $RAIZ_SITIO_nohttp . "images/procesos_prod/";
	if (isset($_SESSION["Empresa_ID"])) {
		if (is_file($target_dir . $_SESSION["Empresa_ID"] . '.jpg')) {
			$imagen = $RAIZ_SITIO . "images/procesos_prod/" . $_SESSION["Empresa_ID"] . '.jpg';
			$con_imagen = "si";
		} else {
			$imagen = $RAIZ_SITIO . "images/procesos_prod/" . 'perfil.png';
			$con_imagen = "no";
		}
	} else {
		$imagen = $RAIZ_SITIO . "images/procesos_prod/" . 'perfil.png';
	...
	if ($_SESSION['tipo'] == "Alumn") {
		if ($_SESSION['Puesto_ID'] == 5 OR $_SESSION['Puesto_ID'] == 10) {
			$le_compete = "";
		}
	}
?>
			<div class="card shadow mb-1">
				<div class="card-header">
					Sesion 6 <i class="fas fa-angle-right fa-lg fa-fw"></i> Factibilidad Técnica <i class="fas fa-angle-right fa-lg fa-fw"></i> Proceso de Producción
				</div>
				<div class="card-body px-5">
					<h5 class="card-title">Proceso de Producción</h5>
					<div class="card-text pl-5 pr-5">
						<p class="text-justify">Para definir los <strong>Procesos de Producción</strong> de nuestra Empresa, debemos considerar una serie de pasos a seguir en la consecución de una meta. En el caso de una Empresa, estos deberán responder al cumplimiento de los objetivos y metas planteados en cada una de las áreas que la conforman.</p>
						<div class="card bg-success border-success px-3 py-3 mb-3">
							<p class="text-justify text-white"><b>Diagrama de Flujo</b>.</p>
							<p class="text-justify text-white">Es una representación simbólica que describe la secuencia de las fases de unproceso. Su objetivo es mejorar la visualización del proceso, siendo una herramienta útil para verificarcómo los variados pasos del proceso están relacionados entre sí y descubrir eventuales fallas deprocedimiento en la secuencia de operaciones.</p>
							<p class="text-justify text-white">Un <b>Diagrama de Flujo</b> muestra la secuencia cronológica de todas las operaciones, inspecciones, tiempos permitidos y materiales que se utilizan en un proceso de manufactura o de negocios, desde la llegada de la materia prima hasta el empaquetado del producto terminado.</p>
						</div>

						<p class="text-justify">A continuación, te presentamos algunos ejemplos de un diagrama de flujo de un proceso productivo:</p>
						<div class="row justify-content-center">
							<div class="d-none col-lg-2"></div>
							<div class="col-12 col-lg-8">
								<img src="../sesiones/images/6.5_proceso de produccion.png" alt="diagrama de flujo" class="img-fluid">
							</div>
							<div class="d-none col-lg-2"></div>
						</div>
						<br>
						<p class="text-justify">Realicen en equipo como establecerían un diagrama de flujo para la elaboración de su producto o servicio. La dirección de Producción debe subir su diagrama. En este link encontrarás más ejemplos de apoyo: <a href="https://concepto.de/diagrama-de-flujo/" target="_blank">Ejemplos de Diagramas de Flujo</a></p>

						<p class="text-justify">El <b>Director de Producción y sus Gerentes</b>, deberán elaborar y subir su Proceso de Producción.</p>
						...
						<form action="<?php echo $RAIZ_SITIO; ?>scripts/alumno/registrar_6_5.php" method="post" class="mt-1" enctype="multipart/form-data">
							...
							A continuación, sube una imagen de tu <strong>Proceso de Producción</strong><br><br>
							...
							<div class="col-12 text-center">
								...
								<div class="custom-file mb-3 mt-1 col-6">
									<label class="control-label text-dark-gray">Proceso de Producción:</label>
									<input id="upload" name="upload" type="file" class="custom-file-input" onchange="readUpload(this);" <?php echo $_SESSION['enable_disable']; ?> <?php echo $le_compete; ?>>
							...
							<div class="form-row pb-1">
								...
								<div class="form-group col-8">
									<?php if ($_SESSION['tipo'] == "Alumn" AND isset($_SESSION["sesion6_5"]) AND $_SESSION["sesion6_5"]==1) { ?><i class="fas fa-exclamation-circle fa-lg fa-fw faa-burst animated"></i><?php } ?>
							...
							<div class="row pb-1">
								<div class="col text-center">
									<?php if ($_SESSION['tipo'] == "Volun") {?>
										<button type="submit" class="btn btn-warning text-center px-5 my-2" name="btn_nuevo_proceso" id="btn_nuevo_proceso" disabled>Registrar comentario</button>
									<?php } else if ($_SESSION['tipo'] == "Alumn") { ?>
										<button type="submit" class="btn btn-warning text-center px-5 my-2" name="btn_nuevo_proceso" id="btn_nuevo_proceso" disabled>Agregar 'Proceso de Producción'</button>
									<?php } ?>
								</div>
							</div>
						</form>
						...
						<div class="row">
							...
							<div class="text-right col-6"><a href="<?php echo $uri_sola . '=5'; ?>" class="btn btn-warning" <?php if ($_SESSION['tipo'] == "Alumn") { ?> onclick="siguiente_seccion(65)" <?php } ?>>Siguiente &nbsp;&nbsp;<i class="fas fa-chevron-right fa-lg fa-fw faa-passing animated"></i></a></div>
						</div>
					</div>
				</div>
			</div>
			...
			<script type="text/javascript">
				...
				<?php } else if ($_SESSION['tipo'] == "Alumn") { ?>
					$(document).ready(function(){
						...
						$.ajax({
						  data:  parametros,
						  url: '../alumno/ajax/empresas_info_s6_5_data.php',
						  type: 'post',
						  success: function(data)
						  {
							...
							if (array.coments_asesor != "") {
								$("#btn_nuevo_proceso").text("Actualizar 'Proceso de Producción'");
							}
					...
					function readUpload(input) {
						if (input.files && input.files[0]) {
							...
							reader.onload = function (e) {
								...
								$('#btn_nuevo_proceso').prop('disabled', false);
							};
				...
				if ($_SESSION['tipo'] != "Alumn") { ?>
					function filtro_empresa(){
						...
						$.ajax({
							url: '../images/procesos_prod/'+Empresa_ID+'.jpg',
							success: function(data){
								$("#image_profile").attr("src","../images/procesos_prod/"+Empresa_ID+ ".jpg");
								$(".img_container").addClass("zoom");
								ind_imagen = 1;
							...
							error: function(data){
								$("#image_profile").attr("src","../images/procesos_prod/perfil.png");
								$(".img_container").removeClass("zoom");
								ind_imagen = 2;
							},
						})
						...
						function defaultImage(e){
							e.target.src= "<?php echo $RAIZ_SITIO . "images/procesos_prod/perfil.png";?>";
							$(".img_container").removeClass("zoom");
							ind_imagen = 0;
						}
						...
						$.ajax({
						  ...
						  success: function(data)
						  {
							...
							if (array.coments_asesor != "") {
								<?php if ($_SESSION['tipo'] == "Volun") {?>
									$("#btn_nuevo_proceso").text("Actualizar comentario");
									if (Empresa_ID > 0) {
										$('#btn_nuevo_proceso').prop('disabled', false);
									}
								<?php } ?>
							...
							if (Empresa_ID == 0) {
								$('#btn_nuevo_proceso').prop('disabled', true);
							}
						  }
						});
					}
