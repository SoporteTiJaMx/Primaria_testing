...
//https://emprendedoresyempresarios.org.mx/scripts/acceso_back.php
//Coordinadora Colbach 13: "Selene78", 1 equipo
//Coordinadora CBTIS 66: "GABBYVERCB66", 4 equipos
//Coordinadora CBTIS 67: "Ezequiel Juárez Hernández", 1 equipo
//Ariadne Onofre Romero: "Ariadne Onofre Romero", alumna con 65 estrellas
//Yaaziel Yehonadav Franco jimenez: "yaaziel", alumna con 65 estrellas
//Kiary Naleli Molina Hernández Hernández: "KiaryHdez", alumna con 55 estrellas
//Asesor 1: "abraham_vs"
//Asesor 2: "avelasco"
//"Athabeli José", "MONSECHL", "RuthTerrero6", "SarahiRein" de CBTIS 90
//
	$nombre = "GABBYVERCB66";
			...
			if($tipo == 'Sadmin'){
				...
				if ($Institucion_ID != '') {
					...
					if (isset($datos[0]) AND $datos[0]>0) {
						...
						/*$datosS1 = mysqli_fetch_row(mysqli_query($con, "SELECT count(*) FROM empresas_info_s1 LEFT JOIN empresas ON empresas_info_s1.Empresa_ID = empresas.Empresa_ID WHERE Escuela_ID=" . $_SESSION["Escuela_ID"] . " AND empresas.Empresa_estatus='Activa' AND (empresas_info_s1.act_alumno=1 OR empresas_info_s1.act_asesor=1)"));
						if ($datosS1[0]>0) {
							$_SESSION["sesion1"] = 1;
					...
					if (isset($datos[0]) AND $datos[0]>0) {
						...
						$_SESSION["Escuela_maps"] = $datos[18];
						$_SESSION["Escuela_web"] = $datos[19];
						$_SESSION["subseccion_general"] = 45;
						$seccion = "../coordinador.php";
						/*$datosS1 = mysqli_fetch_row(mysqli_query($con, "SELECT count(*) FROM empresas_info_s1 LEFT JOIN empresas ON empresas_info_s1.Empresa_ID = empresas.Empresa_ID WHERE Escuela_ID=" . $_SESSION["Escuela_ID"] . " AND empresas.Empresa_estatus='Activa' AND (empresas_info_s1.act_alumno=1 OR empresas_info_s1.act_asesor=1)"));
						if ($datosS1[0]>0) {
							$_SESSION["sesion1"] = 1;
