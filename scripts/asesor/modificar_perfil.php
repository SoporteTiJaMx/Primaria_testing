...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	$carrera = (isset($_POST["carrera"])) ? sanitizar($_POST["carrera"]) : null;
	$trabajo = (isset($_POST["trabajo"])) ? sanitizar($_POST["trabajo"]) : null;
	$puesto = (isset($_POST["puesto"])) ? sanitizar($_POST["puesto"]) : null;
	$correo = (isset($_POST["correo"])) ? sanitizar($_POST["correo"]) : null;
	$tel = (isset($_POST["tel"])) ? sanitizar($_POST["tel"]) : "";
	$cel = (isset($_POST["cel"])) ? sanitizar($_POST["cel"]) : "";
	...
	if (isset($_POST["csrf"]) && $_POST["csrf"] == $_SESSION["token"]) {
		$query = "UPDATE asesores SET Asesor_nombre=?, Asesor_ap_paterno=?, Asesor_ap_materno=?, Asesor_cumple=?, Asesor_email=?, Asesor_tel=?, Asesor_cel=?, Asesor_trabajo=?, Asesor_puesto=?, Asesor_estatus=?, Carrera_ID=? WHERE User_ID=?";
		if ($stmt = $con->prepare($query)) {
			$estatus = 1;
			$stmt->bind_param("sssssssssiii", $nombre,$ap_paterno,$ap_materno,$cumple,$correo,$tel,$cel,$trabajo,$puesto,$estatus,$carrera,$_SESSION["User_ID"]);
			$estado = $stmt->execute();
		}
		if ($estado) {
			...
			echo "<META HTTP-EQUIV='REFRESH' CONTENT='5;URL=../../asesor/perfil.php'>";
					...
					<div class="col-6 mx-auto">
						<div class="card shadow">
							<div class="card-body">
								...
								<div class="text-right mt-5"><a href="../../asesor/perfil.php" class="btn btn-warning">Ir</a></div>
							</div>
						</div>
					</div>
				</div>
