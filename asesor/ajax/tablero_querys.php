...
if(
	...
	if ($Empresa_ID > 0) {
		...
		$query = "SELECT empresas.Empresa_ID, empresas.Empresa_nombre, empresas.Empresa_producto, empresas.Empresa_estatus, escuelas.Escuela_nombre FROM empresas LEFT JOIN escuelas ON escuelas.Escuela_ID = empresas.Escuela_ID WHERE empresas.Asesor_ID=? AND empresas.Empresa_estatus='Activa' order by empresas.Empresa_nombre";
	} else {
		$query = "";
	}
	if($stmt = $con->prepare($query)){
		if ($Empresa_ID > 0) {
			...
			$stmt->execute();
			$stmt->bind_result($Empresa_ID, $Empresa_nombre, $Empresa_producto, $Empresa_estatus, $Escuela_nombre);
			$tabla = "
				<table id='Datos-filtrados' class='table table-hover' style='width:100%'>
					<thead>
						<tr>
