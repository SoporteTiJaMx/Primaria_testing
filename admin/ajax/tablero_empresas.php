...
if(
	...
	$query = "SELECT * FROM empresas_puntos WHERE Empresa_ID IN (SELECT Empresa_ID FROM licencia_empresa WHERE Licencia_ID=? AND Empresa_ID IN (SELECT Empresa_ID FROM empresas WHERE Empresa_estatus='Activa'))";
	if($stmt = $con->prepare($query)){
		...
		while ($stmt->fetch()) {
			...
			$tabla.="<tr>
				<td class='text-center'>" . $Empresa_ID . "</td>
