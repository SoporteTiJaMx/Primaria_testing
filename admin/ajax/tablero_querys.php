...
if(
	...
	if ($Empresa_ID > 0) {
		...
		$query = "SELECT empresas.Empresa_ID, empresas.Empresa_nombre, empresas.Empresa_producto, empresas.Empresa_estatus, asesores.Asesor_nombre, asesores.Asesor_ap_paterno, escuelas.Escuela_nombre FROM empresas LEFT JOIN escuelas ON escuelas.Escuela_ID = empresas.Escuela_ID LEFT JOIN licencia_empresa ON licencia_empresa.Empresa_ID = empresas.Empresa_ID LEFT JOIN asesores ON asesores.Asesor_ID = empresas.Asesor_ID WHERE licencia_empresa.Empresa_ID IN (SELECT Empresa_ID FROM licencia_empresa WHERE Licencia_ID=?) AND empresas.Empresa_estatus='Activa' order by empresas.Empresa_nombre";
	} else {
		$query = "";
	}
	if($stmt = $con->prepare($query)){
		if ($Empresa_ID > 0) {
			...
			$tabla = "
				...
				while ($stmt->fetch()) {
					...
					/*} else if($Alumno_subseccion<60){
						$Alumno_Sesion=5;
						if ($Alumno_subseccion==51) {
							$Seccion5 = "5.1 Áreas de una Empresa";
						} else if ($Alumno_subseccion==52) {
							$Seccion5 = "5.2 Detección de Habilidades";
						} else if ($Alumno_subseccion==53) {
							$Seccion5 = "5.3 Procesos de Selección";
						} else if ($Alumno_subseccion==54) {
							$Seccion5 = "5.4 Diseño Organizacional";
						} else if ($Alumno_subseccion==55) {
							$Seccion5 = "5.5 Actividades";
						}
						$Seccion1 = "Sesión Concluida";
						$Seccion2 = "Sesión 1 Concluida";
						$Seccion3 = "Sesión 2 Concluida";
						$Seccion4 = "Sesión 3 Concluida";
						$Seccion5 = "Sesión 4 Concluida";*/
					} else {
			...
			$stmt->execute();
			$stmt->bind_result($Empresa_ID, $Empresa_nombre, $Empresa_producto, $Empresa_estatus, $Asesor_nombre, $Asesor_ap_paterno, $Escuela_nombre);
			$tabla = "
				<table id='Datos-filtrados' class='table table-hover' style='width:100%'>
					<thead>
						<tr>
				...
				$tabla.= "</tr>
				 ...
				 while ($stmt->fetch()) {
					...
					$tabla.="<tr>
						<td class='align-middle'>" . $Empresa_ID . "</td>
