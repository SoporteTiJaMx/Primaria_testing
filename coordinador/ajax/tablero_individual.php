...
if(
	...
	if ($Empresa_ID > 0) {
		...
		$query = "SELECT alumnos.Alumno_nombre, alumnos.Alumno_ap_paterno, alumnos.Alumno_ap_materno, alumnos.Alumno_score, alumnos.Alumno_subseccion, alumnos.Alumno_estatus, puestos.Puesto_nombre, empresas.Empresa_ID, empresas.Empresa_nombre, escuelas.Escuela_nombre FROM empresas INNER JOIN escuelas ON escuelas.Escuela_ID = empresas.Escuela_ID INNER JOIN licencia_empresa ON licencia_empresa.Empresa_ID = empresas.Empresa_ID INNER JOIN alumnos ON empresas.Empresa_ID = alumnos.Empresa_ID INNER JOIN puestos ON alumnos.Puesto_ID=puestos.Puesto_ID WHERE licencia_empresa.Empresa_ID IN (SELECT Empresa_ID FROM licencia_empresa WHERE LIcencia_ID=?) AND empresas.Empresa_estatus='Activa' AND escuelas.Escuela_ID = " . $Escuela_ID. " ORDER BY alumnos.Alumno_score DESC";
	} else {
		$query = "";
	}
	if($stmt = $con->prepare($query)){
		if ($Empresa_ID > 0) {
			...
			$tabla = "
				<table id='Datos-filtrados2' class='table table-hover' style='width:100%'>
					<thead>
						<tr>
